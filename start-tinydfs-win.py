...
def kill_tinydfs_processes():
	...
	if os_system == "Windows" :
		java_processes = os.popen('tasklist /FI "IMAGENAME eq java.exe"').read()
	...
	for line in line_java_processes:
		if os_system == "Linux" :
			if 'MasterMain' not in line and 'SlaveMain' not in line:
				continue
		...
		for word in words:
			...
			if process_id != 0:
				...
				elif "Windows" in os_system:
					command = 'taskkill /f /im ' + str(process_id)
...
if __name__ == '__main__':
	...
	if "Windows" in os_system:
		command_with_parameters = 'START /B ' + command_with_parameters
	elif "Linux" in os_system:
		command_with_parameters = command_with_parameters  + ' &'
